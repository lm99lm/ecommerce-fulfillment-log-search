...
> Design notes for **ecommerce-fulfillment-log-search** — a runnable python example that Ship structured order fulfillment events and search the job trail from Python.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.logs.ingest(...)`).
- **Feature code** — the actual task: ecommerce-job-logs.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `logs.ingest` — mapped to `POST /v1/logs/ingest`.
- `logs.search` — mapped to `GET /v1/logs/search`.
